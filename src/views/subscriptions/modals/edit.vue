...
export default {
    ...
    mounted () {
        window.eventBus.$on('editF', (result) => {
            if (!_.isEmpty(result)) {
    ...
    methods: {
        async save () {
            ...
            this.startLoading()
            let formula_uuid = this.formula.uuid.replace('-', '')
            const response = await this.$api.put(`api/admin/formula/${formula_uuid}/update`, this.ghost)
                .catch(error => {
                    console.log('Error', error.response.data.name)
                    this.$swal.error(this.$translate.text('Error'), this.$translate.text(error.response.data.name))
                })
